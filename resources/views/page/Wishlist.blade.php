            ...
            <div class="col-md-2 mb-4">
                <div class="card h-100 position-relative border-0 shadow-sm">
                    ...
                    <form action="{{ route('wishlist.delete' , $wishlist->id) }}" method="POST">
                        @csrf
                        @method('DELETE')
                        <button type="submit" class="btn btn-sm position-absolute top-0 end-0 m-2" style="background: none; border: none; font-size: 18px;">×</button>
                    <button class="btn btn-sm position-absolute top-0 end-0 m-2" onclick="removeFromWishlist({{ $wishlist->id }})" style="background: none; border: none; font-size: 18px;">×</button>
                    </form>

                    <!-- Product Image -->
                    <img src="{{ asset('storage/' . $product->image) }}" class="card-img-top" style="height: 220px; object-fit: contain;" alt="{{ $product->name }}">
                    ...
                    <div class="card-body text-center p-2">
                        ...
                        <div class="d-flex justify-content-center align-items-center gap-2">
                            ...
                            <span class="text-muted text-decoration-line-through" style="font-size: 13px;">{{ $product->mrp }}</span>
                            <span class="text-danger" style="font-size: 13px;">
                              ({{ $product->mrp > 0 ? round((($product->mrp - $product->selling_price) / $product->mrp) * 100) : 0 }}% OFF)
                            </span>
